...
flowchart LR
    ...
    M6 --> M7[M7 接已有终端<br/>对话 · 面板可写] --> M8[M8 总控面板]
```

依赖关系：M4 与 M5 相互独立，可按兴趣调换顺序；M6 的面板部分可与 M4/M5 并行。
M7/M8 是 M0–M6 全部完成后，用户在试用中提出的二期需求（见文末各自小节）。
...
## M8 · 一个面板管所有机器（用户追加的需求）

用户在问「一台机器上多个 Claude Code、几台机器上不同 Claude Code 分别怎么用，
面板是一台有就行还是每台都有」之后给出了明确要求：
**我是想一个总控面板管所有机器的这样比较方便**。

此前每台机器的面板只看自己 —— 机器一多就没人看了，这是可用性上的真问题。

- [x] `web/cluster.py`：本机 + 所有已信任对端汇成一个节点列表
- [x] `GET /node/summary`：只读，给已信任对端看，HMAC 签名的请求
- [x] `security/signing.py` 的 `sign_request` / `verify_request`
- [x] `anthill serve` 定期写 `.anthill/status.json`
- [x] 面板前端改成集群视图：拓扑按机器分组，任务与消息流加「在哪台机器上」

三个关键取舍：

1. **约定一个汇合点，而不是让面板去逐个文件拉。** 跨机没有共享文件系统，
   逐个文件走 SFTP 又太碎。所以每个节点定期把自己的快照写成一个
   `.anthill/status.json`，总控只需要取那**一个**文件 ——
   剩下的只是「怎么把一个文件从那边拿过来」，而这条路每种传输本来就有：
   LAN 走 HTTP，SSH 走 SFTP。**SSH 那侧仍然不用开任何端口**，这一点不能因为
   做面板而破例。
2. **读也要认证，而且用同一把密钥。** 新造一套令牌就是新造一处会过期、会泄露、
   会忘记轮换的东西。共享密钥已经在那儿了 —— 签 `节点 + 路径 + 时间戳`，
   同样吃 5 分钟防重放窗。**未信任的节点只出现在拓扑里，总控不去读它的状态**：
   「发现 ≠ 可通信」这条线在读取方向上同样成立。
3. **一台连不上不能把整页卡住。** 分布式面板最常见的坏法就是一台机器挂了、
   整个页面转圈。所以所有节点并发拉取、各自超时，拿不到就标成「连不上」
   并把原因写出来 —— 页面上看得见「哪台坏了、为什么」，比看不见有用得多。
   实测拔掉一台之后，总控页面 0.17 秒返回，其余节点照常。

联调时揪出一个只有真跑起来才会看见的 bug：**WebSocket 推的是本机快照，
会把总控视图里其他机器抹掉** —— 页面打开时三台机器都在，一秒后只剩本机。
改成两路数据合成一张图：WS 只更新本机那一格（保留 kill -9 之后立刻变灰的手感），
全集群走 5 秒一次的 `api/cluster`。顺带补上 `tests/js/panel_check.mjs` ——
页面上那段 JS 此前没有任何东西跑过它，而合并逻辑恰恰是最容易悄悄坏掉的地方。

### 合并前那轮 review 揪出来的（这一轮收获最大）

写完之后按惯例过了一轮 code review，结果比前几个里程碑都重 ——
**因为「多了一个数据来源」这件事本身就改变了威胁模型**，而我在写的时候
还在用单机时代的假设。四条最值得记：

1. **原来是本机算出来的数字，现在是别人给的。** `queue` / `dead` / `round`
   这几个字段此前来自 `len(mailbox.list_new())`，前端插值时没转义也无所谓；
   接上远端之后它们变成对端 JSON 里的任意值，而面板开了写权限时
   「在面板里执行 JS」约等于「在这台机器上执行命令」。
   一次性的答案是补三个 `esc()`；**真正的答案是在进程边界上校验** ——
   `web/status.py` 用 pydantic 把整份快照过一遍，声明成整数的字段收到字符串
   就整份丢掉、把那台机器标成不可用。两道都要有。
2. **「读得到文件」不等于「那台机器活着」。** SSH 那条路是直接读对方磁盘上的
   `status.json`；那台机器上的 serve 被 kill 之后文件还在、读得到 ——
   不看时间戳的话，一台死了一周的机器会一直显示成绿灯、Agent 全在跑。
   这条恰好把这个里程碑的招牌承诺（kill -9 之后立刻变灰）反过来了。
3. **一个「看看状态」的 GET 是有副作用的。** 每次请求可能触发 N 次 SSH 登录；
   浏览器每 5 秒轮询、多开一个标签页就翻倍，足够在学校服务器上撞 `MaxStartups`。
   于是加了按节点的缓存与「有旧数据就先给旧的、后台去刷」——
   顺带把「一台连不上不会把整页卡住」从「有超时」升级成「根本不等」。
4. **写状态失败把整个 serve 带走了。** 那段循环只接 `OSError`，
   而 `atomic_write` 抛的是 `MailboxError`；磁盘一满，任务崩、`serve` 跟着退出，
   退出码还是 0。少一份状态是小事，节点掉线是大事 —— 现在接住所有异常，
   并补上 `serve.task_crashed`：后台任务是炸掉才停的，必须说出来。

另外还收了几条：请求签名的材料加域分隔标签、时间窗从 5 分钟收到 30 秒
（读请求是同步的，又没有 seen.db 兜底）、认证代码不再被畸形 HTTP 头打成 500、
共享出去的快照不带 peers 列表、跨零点时消息流的排序键。
