...
flowchart LR
    ...
    M6 --> M7[M7 接已有终端<br/>对话 · 面板可写] --> M8[M8 总控面板] --> M9[M9 默认可见<br/>PIN 配对 · 面板对话] --> M10[M10 面板即控制面]
```

依赖关系：M4 与 M5 相互独立，可按兴趣调换顺序；M6 的面板部分可与 M4/M5 并行。
M7–M10 是 M0–M6 全部完成后，用户在试用中提出的二期需求（见文末各自小节）。
...
用户用起总控面板之后提了三条，都指向同一件事 —— 机器一多就嫌麻烦：
**serve 之后就该能互相看见**、**面板要能配别的机器和发起任务**、
**能在面板上跟任意一个 agent 直接对话**。

- [x] `discovery.enabled` 默认翻成 true
- [x] `security/pairing.py` + `pair_client.py`：六位 PIN 码配对（SPAKE2）
- [x] `web/chat.py` + 面板的「对话」标签页
- [x] `web/admin.py` + `web/remote.py`：远端读写 node.toml

### 四个想清楚了才动手的点

1. **「可见」和「互信」是两件事，只放开前一件。** 默认静默是 M4 的招牌，
   但实际用下来「同网段两台机器要先手动互相告知地址」太劝退，
   而广播包里本来就只有节点名、Agent 名单、地址。
   放开可见，**发现 ≠ 可通信那条线一个字不动** —— 要互投消息仍然必须
   有人在两边各看一眼、核对指纹。彻底隐身的路也一直留着。

2. **短口令要安全，就必须用 PAKE，没有中间地带。** 直觉方案是「用 PIN 加密密钥
   发过去」，但六位数字离线暴破是秒级的事，抓一个包就完了。
   spake2（magic-wormhole 那套）让双方**各自推导**密钥：线路上既没有密钥，
   也没有可离线暴破的密文。剩下的在线猜测，用「一个窗口只有一次机会」堵死。
   附带一个坑：**SPAKE2 在口令不符时不报错**，只是各得一把不同的钥匙 ——
   所以必须再加一轮密钥确认，否则 PIN 打错会配成「看起来成功了、
   之后每条消息都验签失败」，是最难查的那种状态。

3. **发出去的信不在自己的邮箱里。** 做面板对话时才意识到：收到的信躺在 `cli` 的
   邮箱里，可发出去的那条被投到对方邮箱去了，本机 outbox 只是投递途中的暂存。
   要在页面上看到一来一回，就得在发的时候自己记一笔 —— 这是「一切皆邮箱」
   这个设计的一个自然代价，不是 bug。

4. **面板改别台机器的配置：把代价说清楚，然后让机器主人自己决定。**
   能改 node.toml ≈ 能在那台机器上执行命令（加一个带 `run_shell` 的 Agent 就行）。
   我原本推荐走 M5 那套 `approvals/` 审批流，用户明确选了直连。
   于是做成直连，但**接收端要一次性显式打开**（`remote_admin`）——
   这不是把选择打折：用户选的是「直连 vs 每次点头」，这个开关是一次性的，
   默认关只是为了别让任何 clone 这个仓库的人在不知情时把机器敞开。
   开启后每一次读写都写审计日志。
...
起因是用户在一个空目录里跑 `anthill serve`，撞上「先跑一次 `anthill init`」。
他的原话：**「启动之后可以通过面板来创建工作区什么的，
单机的话也能直接用 web 面板操控」**。

那条错误信息本身就是问题：它把「先在终端做点什么」当成了前提，
而这正是要去掉的东西。

- [x] `core/workspace.py`：建工作区不再只长在 `init` 里
- [x] `serve` 找不到工作区就自己建一个
- [x] 面板上加 / 删 Agent（`web/agents.py`）
- [x] 面板上启 / 停 agentd

### 联调揪出的三个 bug，一个比一个深

1. **面板加完 Agent，它在面板上根本不出现。** serve 手里捧着启动那一刻读到的
   config。这是「内存缓存 vs 文件真相」这个坑的第三次 —— 前两次是 peers 列表
   （M4）和 seen.db。node.toml 以前确实是启动期常量，是我们自己把它变成
   运行期可改的（面板加 Agent、远端管理），却忘了把读的那一侧一起改。
2. **比上面更糟的一面：新 Agent 收不了跨机消息。** `/deliver` 用同一份旧 config
   判收件人，直接回 404「本节点没有这个 Agent」—— 对面看着面板上明明有它。
   修法和 peers 一样：`ConfigRef` 按 mtime 重载，改坏了就继续用上一份好的。
3. **加了 Agent 却没给它建邮箱。** 上面两个修完，投递又撞上「邮箱还没建」。
   `init` 会给配置里每个 Agent 建邮箱，而「加一个 Agent」这条新路径没有。
   规则应该是「**配置里有它，就该能收它的信**」，不该等到那个 agentd 第一次启动。

还顺带修了两个显示与退出码的问题：面板把 bridge Agent 显示成 `echo`
（这个 bug 在 CLI 上修过一次，面板又犯一遍 —— 因为判定逻辑有两份，
现在共用 `core/config.brain_of`）；端口被占时 `serve` 退出码是 0，
脚本和 systemd 都会以为起成功了（现在开跑前先探端口，给一句人话 + 退出码 1）。

657 个测试全绿。实测：空目录里跑一条 `anthill serve --panel-write`，
之后全部在面板上完成 —— 加一个 bridge Agent、启动它、启动 echo、派活、
看到回执与结果、再把 echo 停掉。全程没开第二个终端。

## M11 · 远端启停、不再擅自建工作区、多工作区清单

- [x] 面板上启 / 停**别台机器**的 agentd（签名请求，和改配置同一道闸）
- [x] `serve` 不再擅自建工作区：不给 `-w` 就在面板上挑目录，
      免得 `.anthill` 落在人没想要的地方
- [x] 机器级工作区清单（`~/.anthill/workspaces.json`），面板上增删改查

## M12 · 多路复用：一台机器一个 serve、一个端口、管全部工作区

用户的原话：**「每个工作区都要起 serve 吗？不能一个机器上的来管理全部工作区吗？」**

之前每个工作区一个 serve 一个端口，纯属把架构里已有的东西忘了：
**路由键本来就写在信封上**（`to.node`），所以一个进程完全可以照看多个节点 ——
多路复用是一张查表，不是多进程编排。

- [x] `web/context.py`：`NodeContext` + `NodeRegistry`
- [x] `/deliver` 按 `to.node` 分派；面板上的 `?node=` 指本机哪个节点
- [x] `ConfigRef` 按 mtime 重载 —— 「内存缓存 vs 文件真相」这个坑的第三次

## M13 · 面板令牌：没有显示器的机器也能被操作

用户的原话：**「61 机器没有接显示器，根本操作不了。」**

面板原来只认「连接来自回环」。那条判据的真实含义是「你是这台机器的主人」，
在笔记本上成立，在机柜里的服务器上直接崩掉。而全新的无头机器还没配过对，
连节点间那条签名通道也用不上 —— 完全够不着。

- [x] `security/panel_token.py`：令牌存 `~/.anthill/panel-token`（0600），
      **永不从命令行参数取**（`ps` 是所有人都看得见的）
- [x] 令牌等价于「能在那台机器上执行命令」，分量和一把 SSH 私钥同档；
      明文 HTTP 上会被嗅探，不可信网络建议 `ssh -L`
- [x] 有令牌也不放松同源检查

## M14 · 面板重做（温纸风格），以及它其实一直没在浏览器里跑通

- [x] 单页重写：温纸配色 + 技术排版，三级字体分工（等宽只给机器数据）
- [x] **装了真浏览器测试之后才发现：这个面板此前在浏览器里根本没跑通过。**
      页面挂在 `/panel`（没有尾斜杠），`fetch("api/cluster")` 解析成 `/api/cluster`
      —— 404，拓扑一直是空的；而且 `websockets` 从来不是依赖，
      uvicorn 把 WS 升级当 404，实时推送从没通过。**所有测试当时都是绿的。**
- [x] `tests/integration/test_panel_browser.py`：用 playwright 真开一个 chromium

## M15 · 一次外部评审之后的修复

一位评审者实测复现了 4 个确定性 bug，另有 2 个读码确认的架构级问题。
值得记下来的是它们的分布：**几乎全部集中在 M9–M14 这批后期新增的面板/编排代码里**
—— 前面几层建立起来的安全与健壮性基线，后来的代码没跟上。

- [x] 面板 WebSocket 完全没有鉴权（`accept()` 之前一行检查都没有），
      推的却是和 `/api/state` 一模一样的快照
- [x] `STATE_MARK` 漏了 `SKIPPED` —— 并行 DAG 一有分支失败，看板就崩
- [x] 少一个 `f` 前缀，多节点远端启停的路由从来没匹配上过任何请求
- [x] 上下文预算裁剪会产出孤儿 tool 结果 —— 任务越长越容易触发
- [x] 崩溃恢复的真实语义是「最多一次」：`seen.db` 把重放抑制掉了，
      `recover_stale` 是个安慰剂。改成 claimed/completed 两阶段
- [x] 一个伪造的 UDP 包能劫持已信任节点的全部出站消息（`observe` 无条件覆盖 endpoint）
- [x] 能力缺口：补 `edit_file` / `search_text` / `find_files` / `read_file` 分页；
      编排层真的读 `retryable` 了；死信有了 list/retry/drop；
      所有单调增长的目录装上刹车
- [x] CI 从来没跑过浏览器测试（没装 chromium，静默 skip），补上并加 timeout
