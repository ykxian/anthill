...
- 单页 HTML + 原生 JS + WebSocket，无构建链；FastAPI 挂 `/panel`。
- 数据源：agentd 把每条消息事件推给本机 event bus（内存），`/ws` 广播给页面。
- 三块视图：节点/Agent 拓扑（含 peer 状态）、实时消息流（按 thread 折叠）、
  任务看板（计划步骤 × 状态）。
- 只读 + 默认仅绑定 127.0.0.1；确认操作留在 CLI，避免面板成为攻击面。
