...
def codex_session_instructions(layout: NodeLayout, agent: str) -> str:
    ...
    state_list = f"{exe} state list -w {workspace}"
    return (
        ...
        "- `TRUNCATED_WITH_EVIDENCE` 表示全文已卸载；默认不要读取 details 或 "
        "message_file，只有当前任务确实需要时才按路径显式读取，并先核对 SHA-256。\n"
        "- R2/R3 复核只发送新 delta、新 hash、新失败点和上一冻结制品引用，"
        "不要重发完整历史、完整 diff 或完整测试日志。\n"
        "- 节点共享状态不会整份注入对话，也不会因更新唤醒你；需要时按需列出并读取：\n"
        f"  {state_list}\n"
