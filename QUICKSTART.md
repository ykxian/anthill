...
- 会话里让它调 **`anthill_wait`**（阻塞到有人找为止）；
- 不想装 MCP 的话，粘桥接页上那句话就行 —— 那是条跑
  `anthill bridge --wait -1` 的指令，一个道理。

`anthill_inbox` / `anthill_reply` / `anthill_send` 是收发。

**但别在前台等，也别设终点。**

前台阻塞意味着会话整个被占住，你连让它停下来都做不到。所以要**放后台**跑
（Bash 工具的 `run_in_background`）：命令退出时 Claude Code 会自动唤起会话 ——
「有人找你」依然是推过来的，而等待期间会话照常归你用。

超时也要去掉：`--wait 600` 会让它每十分钟醒一次，没消息也得烧掉一轮对话去发现
「没事」，再挂一条。值守本来就该一直挂着，所以用 **`--wait -1`：永不超时**，
有消息才醒。（`--wait 0` 仍是「看一眼就走」—— hook 那条路靠的就是这个语义。）

### 起来就在值守，不用先跟它说话

Claude Code 是回合制的：**没有用户回合，模型不会动**。所以 hook 也好、MCP 也好，
都只能等你先说一句话。唯一能在启动那一刻推动它的，是把提示词当作首个提问传进去：

```bash
ANTHILL_AGENT=cc1 claude "$(/路径/anthill bridge cc1 --prompt -w /你的工作区)"
```

一条命令，起来就在听。提示词是 `--prompt` 现取的，不是内联的 —— 内联的话这段话
每改一次，先前粘出去的命令就旧一次。桥接页上有这条，直接复制。

### 网页上看得见发生过什么

**「对话」那一页就是全部** —— 你跟 Agent 说的，和 Agent 之间说的，在一处。
一段 thread 一张卡，点某一段就接着它往下说（包括**插进两个 Agent 正在进行的
对话**里说一句）。勾「只看 Agent 之间的」把你参与的那些收起来。

这一页非有不可：接上 Claude Code 之后会话几秒就回完了，消息立刻从 `inbox/`
挪进归档。要是只显示「还在等你回」的，页面永远是空的 —— 干得越好越看不出
它在干活。桥接页留着上半截（等你回的）和它自己那一份往来记录。

### 清记录

每段右上角有「删除」，顶上有「清空全部记录」。点之前会先告诉你**要删几条**，
你确认的就是这个数；中间新到了一条消息，这一次整个作废，不会顺手把它带走。

**有一条线它不会替你越过**：`inbox/new/` 里是**送到了但收件的 Agent 还没读**的
消息 —— 删了就是丢件，那个 Agent 醒来后再也看不到。所以默认只删记录，
未读的照实报数、原样留着；真要一起删，确认框里会单独问一次。

> `cli` 那个信箱是个例外：它按设计就没有处理者（只是 `anthill send` 收回执和
> 结果的地方），所以那里的信**永远**是「未读」。想清掉就在确认框里选「连它们一起删」。
