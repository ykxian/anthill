...
面板上就能：加 Agent、把它启动起来、给它派活、跟它对话、改 `node.toml`。
**不用开第二个终端。**
...
## 5. 把已有的 Claude Code 接进来

在面板的「加一个 Agent」里选 **bridge**，然后跟你那个常开的 Claude Code 会话说：

> 盯着 `.anthill/agents/<名字>/bridge/inbox/`，出现新 `.md` 就读，
> 把回复写进 `../outbox/` 下同名的文件。
...
```bash
uv run anthill status                 # 节点总览：谁在跑、积压、死信、对端
...
| `--wait` 一直超时 | 收件的那个 Agent 没跑 —— 面板上点「启动」 |
