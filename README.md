...
- ✅ 中英文 README、CHANGELOG、MIT LICENSE；600 个测试，覆盖率 87%
...
```bash
uv run anthill serve            # 笔记本上：http://127.0.0.1:45778/panel
uv run anthill serve            # 其他每台机器照常起，不用额外配置
```
...
写权限（`--panel-write`）**只对总控这台机器生效**：它发起的任务从本机的 `cli`
出发，跨机那一段仍然走签名投递。总控读得到别人的状态，但改不了别人的配置。
`/panel/api/cluster` 本身也只允许本机访问 —— 它是个有副作用的 GET，
