...
- ✅ **人工审批节点**（`needs_approval`）：审批那套本来就在，编排层却从没用过 ——
  「让人在关键一步之前确认」以前只能靠 Agent 触发危险操作时被策略引擎间接拦下。
  没批就不派，拒了判这一步失败（拒绝是明确答复，不该表现成一直等）
- ✅ **条件分支**（`run_if`）：`upstream_failed` 兜底/回滚、`always` 收尾/清理。
  没有表达式语言 —— 计划是模型生成的，语言越花它写错的概率越高
- ✅ **map 展开**（`for_each`）：在**计划校验阶段**展开成静态 DAG，
  调度、看板、崩溃恢复一行都不用改
- ✅ **每步超时**（`timeout`）：「写代码给 20 分钟、跑个 lint 给 30 秒」
- ✅ **任务模板 / 定时触发 / 完成通知**：`[templates.*]` / `[schedules.*]` / `[notify]`
- ✅ **产物会被校验**：`finish` 声称的路径不存在就当场打回 ——
  以前它只是一串字符串，派活的人拿到手才发现是假的
- ✅ **`--json` 补齐**（status / agent list / peers list / dead list / runs / cost / bridge），
  `anthill runs --follow` 是 `run` 按 Ctrl-C 之后的回程
- ✅ **消息流可过滤、可搜索、可导出**，条数可调
